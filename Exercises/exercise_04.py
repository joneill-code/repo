...
def rc(dna):
    ...
    '''
    return "".join([comp_dict[char] for char in dna][::-1])
...
def percent_decimal(numbers):
    ...
    for num in numbers:
        if num > 1:
            ...
            new_numbers.append(num * 100)
    '''
    return [new_number/100 if new_number >1 else new_number*100 for new_number in numbers]
...
def multiple_proteins_from_rna(rna):
    ...
    return [protein[idx:] for idx, char in enumerate(protein) if char == "M"]
